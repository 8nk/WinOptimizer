...
async def cmd_mark_paid(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not ctx.args:
        ...
        await update.message.reply_text(
            "Выберите клиента для отметки <b>ОПЛАЧЕН</b>:",
...
async def cmd_mark_unpaid(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not ctx.args:
        ...
        await update.message.reply_text(
            "Выберите клиента для отметки <b>НЕ ОПЛАЧЕН</b>:",
            parse_mode="HTML", reply_markup=InlineKeyboardMarkup(buttons)
        )
        return
    cid = ctx.args[0]
...
async def cmd_delete(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id, update.effective_user.username):
        return
    if not ctx.args:
        await update.message.reply_text("🗑 Использование: /delete &lt;client_id&gt;", parse_mode="HTML")
        return
    cid = ctx.args[0]
    ok = api_delete_client(cid)
    if ok:
        await update.message.reply_text(f"🗑 Клиент <code>{cid}</code> удалён.", parse_mode="HTML")
    else:
...
async def cmd_rollback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not ctx.args:
        ...
        targets = [c for c in clients if c.get("status") in ("testing", "unpaid")]
        if not targets:
            await update.message.reply_text("📋 Нет клиентов для отката.")
            return
        buttons = [[InlineKeyboardButton(
            f"⏪ {c.get('pc_name', c['client_id'][:12])}",
        ...
        await update.message.reply_text(
            "⏪ Выберите клиента для <b>ОТКАТА</b>:",
            parse_mode="HTML", reply_markup=InlineKeyboardMarkup(buttons)
        )
        return
    cid = ctx.args[0]
    ...
    if data.startswith("pay_"):
        ...
        if ok:
            ...
            await query.message.reply_text(
                f"✅ Клиент <code>{cid}</code> → <b>ОПЛАЧЕН</b>\n"
        ...
        if ok:
            await query.message.reply_text(
                f"❌ Клиент <code>{cid}</code> → <b>НЕ ОПЛАЧЕН</b>",
                parse_mode="HTML"
            )
        else:
            await query.message.reply_text("❌ Ошибка обновления.")
        ...
        if client:
            ...
            await query.message.reply_text(f"❓ Клиент <code>{cid}</code> не найден.", parse_mode="HTML")
